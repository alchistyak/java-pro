...
public class ProductService {
    ...
    public Product findProductById(Long id) {
        return productRepository.findById(id).orElseThrow(() -> new ServiceException(HttpStatus.NOT_FOUND, "Не найден продукт id=" + id));
    }
...
//        Product product = user.getProducts().stream().filter(x -> x.getId() == id).findFirst().orElseThrow(() -> new ServiceException(HttpStatus.NOT_FOUND, "Не найден продукт id=" + id + " для пользователя userid=" + productDto.userid()));

        Product product = findProductById(productUpdateDto.id());

        product.setAccount(productUpdateDto.account());
        product.setBalance(productUpdateDto.balance());
        product.setType(productUpdateDto.type());
        product = productRepository.save(product);
